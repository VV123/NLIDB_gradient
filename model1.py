...
def construct_graph(mode, env, args):
    ...
    logits, sample_ids = Decoder(args, mode, enc_rnn_out, enc_rnn_state, env.x, emb_Y, emb_out)
    if mode == 'train':
        env.pred = tf.concat((env.y[:,1:], tf.zeros((tf.shape(env.y)[0],1), dtype=tf.int32)), axis=1)
        env.loss = tf.losses.softmax_cross_entropy(tf.one_hot(env.pred, args.output_vocab_size), logits)
